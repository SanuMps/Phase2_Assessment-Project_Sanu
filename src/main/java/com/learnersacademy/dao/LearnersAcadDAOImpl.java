...
public class LearnersAcadDAOImpl implements LearnersAcadDAO {
	...
	public boolean userLogin(String username, String password) throws BusinessException {
		Boolean bool = false;
		try (Connection connection = DbCon1.getConnection()) {
	...
	public boolean deleteClass(Integer classID) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			String sql = "delete from learnersacademy.tab_class where class_id=?";
			PreparedStatement p1 = connection.prepareStatement(sql);
			p1.setInt(1, Integer.valueOf(classID));
	...
	public boolean deleteTeacher(Integer teacherID) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			String sql = "delete from learnersacademy.tab_teacher where teacher_id=?";
			PreparedStatement p1 = connection.prepareStatement(sql);
			p1.setInt(1, Integer.valueOf(teacherID));
			int c = p1.executeUpdate();
			if (c != 1) {
				throw new BusinessException("Insertion Failed");
			} else {
	...
	public boolean addSubject(SubjectObj subject) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			String sql = "insert into learnersacademy.tab_subject (sub_name,sub_lang, class_id, teacher_id) values (?,?,?,?)";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, subject.getSubName());
			preparedStatement.setString(2, subject.getSubLang());
			preparedStatement.setInt(3, subject.getClassId());
			preparedStatement.setInt(4, subject.getTeacherId());
			int c = preparedStatement.executeUpdate();
			if (c != 1) {
				throw new BusinessException("Insertion Failed");
			} else {
	...
	public boolean addStudent(StudentObj student) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			String sql = "Insert into learnersacademy.tab_student(student_name,student_dob, class_id) values(?,?,?)";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, student.getStudentName());
			preparedStatement.setDate(2, student.getStudentDOB());
			preparedStatement.setInt(3, student.getClassID());
			System.out.println("See" + student);
			int c = preparedStatement.executeUpdate();
			if (c != 1) {
				throw new BusinessException("Insertion Failed");
			} else {
	...
	public boolean updSubject(SubjectObj subject) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			String sql = "update learnersacademy.tab_subject set sub_name=?,sub_lang=?,class_id=?,teacher_id=? where sub_id=?";
			PreparedStatement prep = connection.prepareStatement(sql);
			prep.setString(1, subject.getSubName());
			prep.setString(2, subject.getSubLang());
			prep.setInt(3, subject.getClassId());
			prep.setInt(4, subject.getTeacherId());
			prep.setInt(5, subject.getSubjectID());
	...
	public boolean updTeacher(TeacherObj teacher) throws BusinessException {
		...
		{
			String sql = "update learnersacademy.tab_teacher set teacher_name=?, teacher_category=?, experience=? where teacher_id=?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, teacher.getTeacherName());
	...
	public boolean updStudent(StudentObj student) throws BusinessException {
		try (Connection connection = DbCon1.getConnection()) {
			...
			PreparedStatement prep = connection.prepareStatement(sql);
			System.out.println("Update wala" + student);
			prep.setString(1, student.getStudentName());
			prep.setDate(2, student.getStudentDOB());
			prep.setInt(3, student.getClassID());
			prep.setInt(4, student.getStudentId());
			int c = prep.executeUpdate();
			if (c != 1) {
				throw new BusinessException("Insertion Failed");
			} else {
